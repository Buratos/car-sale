@extends("PRODUCTS.Car.crud.create_car.layout")
...
@section("tag_head")
	<head>
		...
		<script src="{{asset('/js/main__create_car.js')}}" type="module"></script>
		{{--  --}}
		<link href="{{asset('/css/my_reset.css')}}" rel="stylesheet"/>
		<link href="{{asset('/css/style.css')}}" rel="stylesheet"/>
		<link href="{{asset('/css/btn_checkbox.css')}}" rel="stylesheet"/>
		<link href="{{asset('/css/create_car.css')}}" rel="stylesheet"/>
		{{--	for debugging --}}
		@if (config('app.debug'))
...
@section("content")
	...
	<section class="content_global_wrapper">
		@include("PRODUCTS.Car.crud.create_car.content")
	</section>
@endsection
